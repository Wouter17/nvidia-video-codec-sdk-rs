    ...
    c.bench_function("encode", |b| {
        b.iter_batched(
            || {
                ...
                // Initialize Vulkan.
                let vulkan_library =
                    VulkanLibrary::new().expect("Vulkan should be installed correctly");
                ...
                let (memory_type_index, physical_device) = instance
                    ...
                    .next()
                    .expect(
                        "There should be at least one GPU which supports a memory type that is \
                         `HOST_VISIBLE`",
                    );
                ...
                .expect(
                    "Vulkan should be installed correctly and `Device` should support \
                ...
                let encoder = Encoder::initialize_with_cuda(cuda_device.clone()).expect(
                    "NVENC API initialization should succeed given that the NVIDIA Video Codec \
                     SDK has been installed correctly",
                );
            ...
            |(file_descriptors, num_bufs, cuda_device, session)| {
                for (i, file_descriptor) in file_descriptors.into_iter().enumerate() {
                    ...
                    let mut external_memory = unsafe {
                        cuda_device
                            .import_external_memory(file_descriptor, (1920 * 1080 * 4) as u64)
                    }
